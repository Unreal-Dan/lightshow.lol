...
import DockManager from './DockManager.js?v=panel-stacks';
...
export default class VortexEditor {
    ...
    document.addEventListener('keydown', async (event) => {
      if (event.key === 'Insert') {
        ...
        this.updatePanel.displayFirmwareUpdateInfo(device, 'N/A',
          latestVersion, communityUrl('/community/downloads'));
  ...
  async getLatestFirmwareVersions(device) {
    ...
    if (this.isLocalServer) {
      ...
      const response = await fetch(communityUrl('/community/downloads/json'));
